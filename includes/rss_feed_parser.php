 ...
 * ---------------------------------------------------
 * RSS Feed Parser (rss_feed_parser.php)
 *****************************************************
...
class FeedParser
{
	...
	private $namespaces     = array('http://purl.org/rss/1.0/' => 'RSS 1.0', 'http://purl.org/rss/1.0/modules/content/' => 'RSS 2.0', 'http://www.w3.org/2005/Atom' => 'ATOM 1');
	...
	private $hasSubTags     = array('IMAGE','AUTHOR','MEDIA:CONTENT'); // List of tag names which have sub tags
	...
	public function parse($url)
	{
		...
		{   
			...
			foreach ($segments as $index=>$data)
			{
				...
				xml_parse($this->xmlParser, $data, $lastPiese) or die(sprintf("XML error: %s at line %d", xml_error_string(xml_get_error_code($this->xmlParser)), xml_get_current_line_number($this->xmlParser)));
			}
	...
	*/
	...
	{
		// Convert all date formats to timestamp...
